...
impl EvalResult {
    ...
    pub fn failed(&self, can_error: bool) -> bool {
        if can_error {
            self.result.is_ok()
                && !matches!(self.result.as_ref(), Ok(Term::Constant { value, .. }) if matches!(value.as_ref(), Constant::Bool(false)))
        } else {
            self.result.is_err()
                || matches!(self.result.as_ref(), Ok(Term::Error { .. }))
                || !matches!(
                  self.result.as_ref(),
                  Ok(Term::Constant { value, .. })
                  if matches!(value.as_ref(), Constant::Bool(true)) || matches!(value.as_ref(), Constant::Unit)
                )
        }
    }
