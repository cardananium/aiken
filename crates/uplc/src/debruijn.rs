...
use crate::ast::{DeBruijn, FakeNamedDeBruijn, GlobalNamedDeBruijn, GlobalIdManager, Name, NamedDeBruijn, Term, Unique};
...
impl Converter {
    pub fn new() -> Self {
        Converter {
            ...
            global_id_manager: GlobalIdManager::new(),
        }
    }
    ...
    /// Convert Name term to GlobalNamedDeBruijn with automatically assigned global IDs
    pub fn name_to_global_named_debruijn(
        &mut self,
        term: &Term<Name>,
    ) -> Result<Term<GlobalNamedDeBruijn>, Error> {
        let converted_term = match term {
            Term::Var(name) => Term::Var(
                GlobalNamedDeBruijn {
                    text: name.text.to_string(),
                    index: self.get_index(name)?,
                    global_id: self.global_id_manager.next_id(),
                }
                .into(),
            ),
            Term::Delay(term) => Term::Delay(Rc::new(self.name_to_global_named_debruijn(term)?)),
            Term::Lambda {
                parameter_name,
                body,
            } => {
                ...
                let body = self.name_to_global_named_debruijn(body)?;
            ...
            Term::Apply { function, argument } => Term::Apply {
                function: Rc::new(self.name_to_global_named_debruijn(function)?),
                argument: Rc::new(self.name_to_global_named_debruijn(argument)?),
            },
            Term::Constant(constant) => Term::Constant(constant.clone()),
            Term::Force(term) => Term::Force(Rc::new(self.name_to_global_named_debruijn(term)?)),
            Term::Error => Term::Error,
            Term::Builtin(builtin) => Term::Builtin(*builtin),
            Term::Constr { tag, fields } => Term::Constr {
                tag: *tag,
                fields: fields
                    .iter()
                    .map(|field| self.name_to_global_named_debruijn(field))
                    .collect::<Result<_, _>>()?,
            },
            Term::Case { constr, branches } => Term::Case {
                constr: Rc::new(self.name_to_global_named_debruijn(constr)?),
                branches: branches
                    .iter()
                    .map(|branch| self.name_to_global_named_debruijn(branch))
                    .collect::<Result<_, _>>()?,
            },
        };
    ...
    /// Convert NamedDeBruijn term to GlobalNamedDeBruijn with automatically assigned global IDs
    pub fn named_debruijn_to_global_named_debruijn(
        &mut self,
        term: &Term<NamedDeBruijn>,
    ) -> Term<GlobalNamedDeBruijn> {
        match term {
            Term::Var(var) => {
                let global_var = GlobalNamedDeBruijn {
                    text: var.text.clone(),
                    index: var.index,
                    global_id: self.global_id_manager.next_id(),
                };
                Term::Var(Rc::new(global_var))
            },
            Term::Delay(term) => {
                Term::Delay(Rc::new(self.named_debruijn_to_global_named_debruijn(term)))
            },
            Term::Lambda { parameter_name, body } => {
                let global_param = GlobalNamedDeBruijn {
                    text: parameter_name.text.clone(),
                    index: parameter_name.index,
                    global_id: self.global_id_manager.next_id(),
                };
                Term::Lambda {
                    parameter_name: Rc::new(global_param),
                    body: Rc::new(self.named_debruijn_to_global_named_debruijn(body)),
                }
            },
            Term::Apply { function, argument } => Term::Apply {
                function: Rc::new(self.named_debruijn_to_global_named_debruijn(function)),
                argument: Rc::new(self.named_debruijn_to_global_named_debruijn(argument)),
            },
            Term::Constant(c) => Term::Constant(c.clone()),
            Term::Force(term) => {
                Term::Force(Rc::new(self.named_debruijn_to_global_named_debruijn(term)))
            },
            Term::Error => Term::Error,
            Term::Builtin(b) => Term::Builtin(*b),
            Term::Constr { tag, fields } => Term::Constr {
                tag: *tag,
                fields: fields.iter().map(|f| self.named_debruijn_to_global_named_debruijn(f)).collect(),
            },
            Term::Case { constr, branches } => Term::Case {
                constr: Rc::new(self.named_debruijn_to_global_named_debruijn(constr)),
                branches: branches.iter().map(|b| self.named_debruijn_to_global_named_debruijn(b)).collect(),
            },
        }
    }

    /// Re-assign global IDs to all variables in an existing GlobalNamedDeBruijn term
    pub fn reassign_global_ids_to_term(
        &mut self,
        term: &Term<GlobalNamedDeBruijn>,
    ) -> Term<GlobalNamedDeBruijn> {
        match term {
            Term::Var(var) => {
                let mut new_var = var.as_ref().clone();
                new_var.global_id = self.global_id_manager.next_id();
                Term::Var(new_var.into())
            },
            Term::Delay(term) => {
                Term::Delay(Rc::new(self.reassign_global_ids_to_term(term)))
            },
            Term::Lambda { parameter_name, body } => {
                let mut new_param = parameter_name.as_ref().clone();
                new_param.global_id = self.global_id_manager.next_id();
                Term::Lambda {
                    parameter_name: new_param.into(),
                    body: Rc::new(self.reassign_global_ids_to_term(body)),
                }
            },
            Term::Apply { function, argument } => Term::Apply {
                function: Rc::new(self.reassign_global_ids_to_term(function)),
                argument: Rc::new(self.reassign_global_ids_to_term(argument)),
            },
            Term::Constant(c) => Term::Constant(c.clone()),
            Term::Force(term) => {
                Term::Force(Rc::new(self.reassign_global_ids_to_term(term)))
            },
            Term::Error => Term::Error,
            Term::Builtin(b) => Term::Builtin(*b),
            Term::Constr { tag, fields } => Term::Constr {
                tag: *tag,
                fields: fields.iter().map(|f| self.reassign_global_ids_to_term(f)).collect(),
            },
            Term::Case { constr, branches } => Term::Case {
                constr: Rc::new(self.reassign_global_ids_to_term(constr)),
                branches: branches.iter().map(|b| self.reassign_global_ids_to_term(b)).collect(),
            },
        }
    }

    /// Convert DeBruijn term to GlobalNamedDeBruijn with automatically assigned global IDs
    pub fn debruijn_to_global_named_debruijn(
        &mut self,
        term: &Term<DeBruijn>,
    ) -> Term<GlobalNamedDeBruijn> {
        match term {
            ...
            Term::Delay(term) => Term::Delay(Rc::new(self.debruijn_to_global_named_debruijn(term))),
            Term::Lambda { parameter_name, body } => {
                let global_param = GlobalNamedDeBruijn {
                    text: "i".to_string(),
                    index: *parameter_name.as_ref(),
                    global_id: self.global_id_manager.next_id(),
                };
                Term::Lambda {
                    parameter_name: Rc::new(global_param),
                    body: Rc::new(self.debruijn_to_global_named_debruijn(body)),
                }
            },
            Term::Apply { function, argument } => Term::Apply {
                function: Rc::new(self.debruijn_to_global_named_debruijn(function)),
                argument: Rc::new(self.debruijn_to_global_named_debruijn(argument)),
            },
            Term::Constant(constant) => Term::Constant(constant.clone()),
            Term::Force(term) => Term::Force(Rc::new(self.debruijn_to_global_named_debruijn(term))),
            Term::Error => Term::Error,
            Term::Builtin(builtin) => Term::Builtin(*builtin),
            Term::Constr { tag, fields } => Term::Constr {
                tag: *tag,
                fields: fields.iter().map(|field| self.debruijn_to_global_named_debruijn(field)).collect(),
            },
            Term::Case { constr, branches } => Term::Case {
                constr: Rc::new(self.debruijn_to_global_named_debruijn(constr)),
                branches: branches.iter().map(|branch| self.debruijn_to_global_named_debruijn(branch)).collect(),
            },
        }
    }

    /// Convert GlobalNamedDeBruijn to NamedDeBruijn (loses global ID information)
    #[allow(clippy::only_used_in_recursion)]
    pub fn global_named_debruijn_to_named_debruijn(
        &mut self,
        term: &Term<GlobalNamedDeBruijn>,
    ) -> Term<NamedDeBruijn> {
        match term {
                         Term::Var(var) => Term::Var(NamedDeBruijn {
                 text: var.text.clone(),
                 index: var.index,
             }.into()),
            Term::Delay(term) => Term::Delay(Rc::new(self.global_named_debruijn_to_named_debruijn(term))),
                         Term::Lambda { parameter_name, body } => Term::Lambda {
                 parameter_name: NamedDeBruijn {
                     text: parameter_name.text.clone(),
                     index: parameter_name.index,
                 }.into(),
                 body: Rc::new(self.global_named_debruijn_to_named_debruijn(body)),
             },
            Term::Apply { function, argument } => Term::Apply {
                function: Rc::new(self.global_named_debruijn_to_named_debruijn(function)),
                argument: Rc::new(self.global_named_debruijn_to_named_debruijn(argument)),
            },
            Term::Constant(constant) => Term::Constant(constant.clone()),
            Term::Force(term) => Term::Force(Rc::new(self.global_named_debruijn_to_named_debruijn(term))),
            Term::Error => Term::Error,
            Term::Builtin(builtin) => Term::Builtin(*builtin),
            Term::Constr { tag, fields } => Term::Constr {
                tag: *tag,
                fields: fields.iter().map(|field| self.global_named_debruijn_to_named_debruijn(field)).collect(),
            },
            Term::Case { constr, branches } => Term::Case {
                constr: Rc::new(self.global_named_debruijn_to_named_debruijn(constr)),
                branches: branches.iter().map(|branch| self.global_named_debruijn_to_named_debruijn(branch)).collect(),
            },
        }
    }

    /// Convert GlobalNamedDeBruijn to DeBruijn (loses global ID and name information)
    #[allow(clippy::only_used_in_recursion)]
    pub fn global_named_debruijn_to_debruijn(
        &mut self,
        term: &Term<GlobalNamedDeBruijn>,
    ) -> Term<DeBruijn> {
        match term {
            Term::Var(var) => Term::Var(var.index.into()),
            Term::Delay(term) => Term::Delay(Rc::new(self.global_named_debruijn_to_debruijn(term))),
            Term::Lambda { parameter_name, body } => Term::Lambda {
                parameter_name: parameter_name.index.into(),
                body: Rc::new(self.global_named_debruijn_to_debruijn(body)),
            },
            Term::Apply { function, argument } => Term::Apply {
                function: Rc::new(self.global_named_debruijn_to_debruijn(function)),
                argument: Rc::new(self.global_named_debruijn_to_debruijn(argument)),
            },
            Term::Constant(constant) => Term::Constant(constant.clone()),
            Term::Force(term) => Term::Force(Rc::new(self.global_named_debruijn_to_debruijn(term))),
            Term::Error => Term::Error,
            Term::Builtin(builtin) => Term::Builtin(*builtin),
            Term::Constr { tag, fields } => Term::Constr {
                tag: *tag,
                fields: fields.iter().map(|field| self.global_named_debruijn_to_debruijn(field)).collect(),
            },
            Term::Case { constr, branches } => Term::Case {
                constr: Rc::new(self.global_named_debruijn_to_debruijn(constr)),
                branches: branches.iter().map(|branch| self.global_named_debruijn_to_debruijn(branch)).collect(),
            },
        }
    }
}
